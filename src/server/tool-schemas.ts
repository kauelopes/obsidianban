...
const CARD_ITEM_SCHEMA = {
  ...
  properties: {
    ...
    status:   { type: 'string', description: "column slug — must match project columns exactly. Default columns use hyphens: 'backlog', 'todo', 'in-progress', 'review', 'done'. Use 'in-progress' (hyphen), NOT 'in_progress' (underscore)" },
...
export const TOOL_SCHEMAS: Record<string, Schema> = {
  ...
  kanban_create_card: {
    ...
    properties: {
      ...
      type:        { type: 'string', enum: ['task', 'feature', 'bug', 'chore'] },
      sprint_id:   { type: 'string' },
      status:      { type: 'string', description: "column slug — defaults to 'backlog'. New projects use: backlog, todo, in_progress, review, done. Legacy hyphenated form 'in-progress' is also accepted." },
      priority:    { type: 'string', enum: ['low', 'medium', 'high', 'critical'] },
      tags:        { type: 'array', items: { type: 'string' }, maxItems: 20 },
      due_date:    { type: 'string', description: 'YYYY-MM-DD' },
      assigned_to: { type: 'string' },
  ...
  kanban_update_card: {
    ...
    properties: {
      ...
      title:       { type: 'string', maxLength: 200 },
      status:      { type: 'string', description: "column slug — e.g. 'in_progress' (underscore). Legacy 'in-progress' (hyphen) also accepted." },
      priority:    { type: 'string', enum: ['low', 'medium', 'high', 'critical'] },
      tags:        { type: 'array', items: { type: 'string' }, maxItems: 20 },
  ...
  kanban_move_card: {
    ...
    properties: {
      id:        { type: 'string' },
      version:   { type: 'integer', minimum: 1 },
      to_status: { type: 'string', description: "column slug — e.g. 'in_progress' (underscore). Default columns: backlog, todo, in_progress, review, done. Legacy 'in-progress' (hyphen) is also accepted." },
      ...TOKEN_FIELDS,
    },
    additionalProperties: false,
  },
  ...
  kanban_pick_next: {
    ...
    properties: {
      ...
      sprint_id:   { type: 'string' },
      assigned_to: { type: 'string' },
      status:      { type: 'string', description: "column to pick from — defaults to 'todo'. Cards in 'backlog' are NOT considered unless you pass status='backlog'. To pick work, cards must first be moved from backlog to todo (e.g. via kanban_add_to_sprint with move_to_todo=true). When card is null, check the 'reason' field: 'no_todo_cards' means there are backlog_count cards in backlog waiting to be promoted, 'all_blocked' means all todo cards have unmet dependencies, 'empty' means the sprint has no cards." },
    },
    additionalProperties: false,
  },
  ...
  kanban_close_sprint: {
    ...
    properties: {
      sprint_id:   { type: 'string' },
      rollover_to: { type: 'string', description: "sprint_id of a planning sprint to receive unfinished cards (required when there are unfinished cards). IMPORTANT: cards in 'done' status are automatically archived on sprint close — they will appear in 'archived' and 'finished' in the response." },
    },
    additionalProperties: false,
  },
}
