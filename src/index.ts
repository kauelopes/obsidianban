...
async function main(): Promise<void> {
  ...
  const tools: ToolDef[] = [
    ...
    { name: 'kanban_log_on_card',          access: 'all',     inputSchema: s['kanban_log_on_card'],          description: 'Append a timestamped log entry to the # Agent Log section. All agent types. Supports markdown and mermaid diagrams. Use this when ONLY logging; use kanban_update_card when updating other fields at the same time.', handler: async (p, c) => cards.logOnCard(p, c) },
    { name: 'kanban_move_card',            access: 'pm',      inputSchema: s['kanban_move_card'],            description: "Move a card to another column. Default columns: backlog, todo, in_progress, review, done (underscore). The legacy hyphenated form 'in-progress' is also accepted. Pass input_tokens/output_tokens to record cost.", handler: async (p, c) => cards.move(p, c) },
    ...
    { name: 'kanban_pick_next',            access: 'all',     inputSchema: s['kanban_pick_next'],            description: "Return the next card ready to work on (no unsatisfied blockers). Only considers cards in 'todo' by default — cards in 'backlog' are NOT returned unless you pass status='backlog'. When card is null, check reason: 'no_todo_cards' = promote backlog cards first via kanban_add_to_sprint with move_to_todo=true.", handler: async (p, c) => cards.pickNext(p, c) },
    ...
    { name: 'kanban_close_sprint',         access: 'pm',      inputSchema: s['kanban_close_sprint'],         description: "Close a sprint; rollover_to required for unfinished cards. IMPORTANT: cards in 'done' are automatically archived — they appear in both 'finished' and 'archived' in the response. Manager or pm agent.", handler: async (p, c) => sprints.closeSprint(p, c) },
  ]
