...
async function main(): Promise<void> {
  ...
  const tools: Array<{ name: string; description: string; handler: ToolFn }> = [
    ...
    { name: 'kanban_create_card', description: "Create a new card. Required: title, type (e.g. feature/bug/task/chore), sprint_id, model. input_tokens and output_tokens are optional and default to 0 at creation — token costs are accumulated on subsequent move/update/log calls. body is optional free-form markdown, write-once at creation; use kanban_log_on_card for all subsequent notes.", handler: async (p, c) => cards.create(p, c) },
    { name: 'kanban_bulk_create_cards', description: 'Create up to 100 cards in one call (partial success); envelope-level token cost is prorated. Same field rules as kanban_create_card — input_tokens/output_tokens default to 0.', handler: async (p, c) => cards.bulkCreate(p, c) },
    { name: 'kanban_update_card', description: 'Update fields of an existing card (optimistic locking). Requires agent_type=pm; dev agents must use kanban_log_on_card. Required: id, version, model, input_tokens, output_tokens — pass token cost accumulated since the last update.', handler: async (p, c) => cards.update(p, c) },
    { name: 'kanban_log_on_card', description: 'Append a timestamped log entry to the card body (# Agent Log section). Available to all agent types including dev. Required: id, version, log_entry, model, input_tokens, output_tokens — pass token cost accumulated since the last update.', handler: async (p, c) => cards.logOnCard(p, c) },
    { name: 'kanban_move_card', description: 'Move a card to another column. Required: id, version, to_status, model, input_tokens, output_tokens — pass the token cost accumulated while working on this card since the last update (usage.input_tokens / usage.output_tokens from the current API response).', handler: async (p, c) => cards.move(p, c) },
    ...
    { name: 'kanban_create_project', description: 'Manager-only — create a project folder and mint an initial pm agent token for it', handler: async (p, c) => admin.createProject(p, c) },
    {
    ...
    { name: 'kanban_create_sprint', description: 'Manager-only — create a sprint (starts in planning; call kanban_start_sprint to activate)', handler: async (p, c) => sprints.createSprint(p, c) },
    { name: 'kanban_start_sprint', description: 'Manager-only — transition a sprint from planning to active; refuses if another sprint is already active in the project', handler: async (p, c) => sprints.startSprint(p, c) },
    { name: 'kanban_list_sprints', description: 'List sprints; filter by status: planning|active|closed|open (planning+active)|all', handler: async (p, c) => sprints.listSprints(p, c) },
    ...
    { name: 'kanban_move_between_sprints', description: 'Manager-only — move cards from sprint_id (source) to target_sprint_id; asserts source membership and same project', handler: async (p, c) => sprints.moveBetweenSprints(p, c) },
    { name: 'kanban_close_sprint', description: 'Manager-only — close a sprint; rollover_to=<planning sprint_id> is required for unfinished cards (archive cards beforehand to discard)', handler: async (p, c) => sprints.closeSprint(p, c) },
  ]
