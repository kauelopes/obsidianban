...
async function main(): Promise<void> {
  ...
  const tools: ToolDef[] = [
    ...
    { name: 'kanban_log_on_card',          access: 'all',     inputSchema: s['kanban_log_on_card'],          description: 'Append a timestamped log entry to the # Agent Log section. Available to all agent types (including dev). Supports markdown and mermaid diagrams. Dev agents: this is your only write tool — use it to record progress. PM agents: prefer kanban_update_card when you need to update other fields at the same time as logging.', handler: async (p, c) => cards.logOnCard(p, c) },
