...
| Agents: MCP is the only permitted write path. No exceptions. Humans: may write via the plugin Kanban UI (which calls MCP) or directly in the Obsidian editor. The file watcher detects and reconciles direct edits within ~600ms (500ms debounce + processing). |
| --- |
...
| Immutable-field protection | File watcher detects and reverts changes to system-managed fields within ~600ms (500ms debounce + processing), preserving human's content edits. |
